...
public class ComputeWeightedSimilarityRunnable implements Runnable{
    ...
    public void run() {
        ...
        try {
            ...
            for(DatasetRow entity: entities){
                SimilarityTool similarityTool = new SimilarityTool(new WeightedAttributeSimilarity(weights,0.1f),0.4f,0.1f,1,0.05f,null);
                ...
                synchronized (lock){
                    ...
                    String query = "UPDATE nearduplicates SET WEIGHTEDSIMILARITY=? where appname=? and crawl=? and state1=? and state2=?;";
    ...
    public ComputeWeightedSimilarityRunnable(Integer slice, Integer start, String folderPath, String datasetDB, Integer numThread) {
        this.slice = slice;
        this.start = start;
